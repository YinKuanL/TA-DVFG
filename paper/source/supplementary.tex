...
aggregation, while same-state interventions on both HGB and MovieLens show that
peer exchange materially improves connected parties' local predictions. Learned
topology is therefore not claimed to be universally necessary for aggregation;
its distinct role is in party-local or joint deployment, where the communication
graph itself changes local inference. A useful mental model is a consortium in which one
...
comparison and nested weak-party scaling; Q2 is addressed by MovieLens; Q3 is
addressed by same-state and selection-only audits on HGB and MovieLens; Q4 by
the deployment-objective audit; and Q5 by the ACM Hard five-seed
...
with the intended deployment interface. These experiments use the strict active-party label-location protocol: only the
active-party training code receives labels directly, while passive parties send
train-node logits and receive logit-space gradients. This is an API-separation protocol, not a
...
all-party local-mean definition. Joint objectives use
\[
s_{\mathrm{joint}}(E)=\lambda s_{\mathrm{active}}(E)
+(1-\lambda)s_{\mathrm{local}}(E),
\]
...
\subsection{MovieLens Mechanism Boundary}

The same-state audit replays each exact \method{} best-validation state and
changes only $K=1\rightarrow0$. The epoch, topology, participant set, degree
vector, reliability vector, prediction snapshot, and readout definition are
held fixed. Five-party Condition A reproduces every existing seed-level
\texttt{adaptive\_graph\_val} reference and has mean ROC-AUC 0.751625.

\begin{table*}[!t]
\centering
\scriptsize
\setlength{\tabcolsep}{4.2pt}
\caption{MovieLens same-state peer-exchange audit over seeds 42--46. Deltas are
$K=1$ minus the exact same state with exchange disabled. Brackets are 95\%
confidence intervals. All all-party and connected-party deltas have
win/tie/loss 5/0/0.}
\label{tab:supp-movielens-exchange}
\begin{tabular}{lccc}
\toprule
Setting & Active AUC $\Delta$ & All-party local mean $\Delta$ & Connected-party mean $\Delta$ \\
\midrule
Five-party & $+0.000163\;[-0.000003,0.000329]$ & $+0.005501\;[0.003310,0.007693]$ & $+0.008456\;[0.006638,0.010275]$ \\
15-party Main & $-0.000017\;[-0.000068,0.000033]$ & $+0.003849\;[0.002924,0.004773]$ & $+0.009935\;[0.008377,0.011492]$ \\
15-party Hard & $+0.000071\;[-0.000102,0.000243]$ & $+0.005281\;[0.003683,0.006879]$ & $+0.013103\;[0.010306,0.015900]$ \\
\bottomrule
\end{tabular}
\end{table*}

The active effect is negligible in all three settings, whereas local gains are
positive for every seed and largest among parties that actually receive peer
messages. This reproduces the HGB mechanism boundary in a real-data benchmark
with heterogeneous predictor families.

For the active-only boundary, Top-Reliability-$q$ and Greedy-Subset-$q$ receive
the same participant cardinality $q=|\mathcal{S}(E)|$ as the corresponding
\method{} state. Selection uses validation information only; test labels are
evaluated after selection, and no selection-only method changes local
predictions.
...
\scriptsize
\setlength{\tabcolsep}{4.0pt}
\caption{MovieLens-15 active-only selection boundary. Communication is mean
transmitted inference scalars.}
\label{tab:supp-movielens-subset}
\begin{tabular}{llrr}
\toprule
...
Greedy-Subset-$q$ matches or exceeds mean active AUC while using about
62--64\% less communication than \method{}. The MovieLens evidence therefore
supports the same interface-dependent conclusion as HGB: active-only deployment
is largely a participant-selection problem, while peer exchange directly changes
party-local inference.
...
\paragraph{Party-count scalability and shortlist prescreening.}
...
We evaluate the proposed two-stage mitigation using the existing Adaptive
Pairwise edge-local utility only as a cheap pre-ranking signal. At each greedy
step, feasible edges are ranked by that utility and whole-graph \method{}
scoring is applied only to the top-$M$ shortlist; the graph-level objective and
all selector defaults are unchanged. The $M=\mathrm{all}$ control exactly
reproduces the exhaustive selector for every $N\in\{15,20,30\}$ and every seed
42--46.
...
\scriptsize
\setlength{\tabcolsep}{4.0pt}
\caption{Top-$M$ prescreening on ACM Hard scalability caches. Accuracy delta is
shortlist minus exhaustive Test@BestVal; speedup and evaluation reduction are
matched within the same run. Bold rows are the nontrivial settings used in the
main-paper claim.}
\label{tab:supp-pruning}
\begin{tabular}{rrrccc}
\toprule
Parties & $M$ & Accuracy $\Delta$ & Speedup & Eval. reduction & Participant agreement \\
\midrule
15 & 5  & $+0.006919$ & $9.39\times$  & 94.80\% & 0.787 \\
15 & 10 & $+0.004942$ & $5.89\times$  & 89.57\% & 0.686 \\
15 & 20 & $+0.006919$ & $3.58\times$  & 79.28\% & 0.712 \\
15 & 40 & $+0.005931$ & $1.84\times$  & 58.74\% & 0.786 \\
\midrule
20 & 5  & $+0.007249$ & $11.08\times$ & 97.19\% & 0.616 \\
20 & 10 & $+0.004613$ & $7.51\times$  & 94.28\% & 0.601 \\
\textbf{20} & \textbf{20} & $\mathbf{+0.000988}$ & $\mathbf{5.22\times}$ & \textbf{88.80\%} & \textbf{0.604} \\
20 & 40 & $+0.007908$ & $3.40\times$  & 77.70\% & 0.719 \\
\midrule
30 & 5  & $+0.005272$ & $28.63\times$ & 98.84\% & 0.534 \\
30 & 10 & $+0.002636$ & $19.81\times$ & 97.61\% & 0.586 \\
30 & 20 & $+0.005272$ & $13.34\times$ & 95.38\% & 0.501 \\
\textbf{30} & \textbf{40} & $\mathbf{-0.002306}$ & $\mathbf{8.46\times}$ & \textbf{90.48\%} & \textbf{0.719} \\
\bottomrule
\end{tabular}
\end{table*}

At 20 parties, top-20 reduces whole-graph evaluations by 88.8\% and yields a
$5.2\times$ speedup while staying within 0.10 accuracy points of exhaustive
search. At 30 parties, top-40 reduces evaluations by 90.5\% and yields an
$8.5\times$ speedup while staying within 0.23 points. The effective shortlist
size is setting-dependent: no nontrivial 15-party row met the main-claim
criterion, so more aggressive or smaller-scale settings are treated as
supplementary diagnostics rather than a universal pruning guarantee. Absolute
seconds from this closure audit are not compared directly with the historical
timing column in Table~\ref{tab:supp-scalability}; the reported speedups are
matched within-run ratios.
...
On HGB, active differences are small and non-significant
($p=.3046/.1778$ on ACM/DBLP), whereas all-party local-mean improvements are
1.85 and 2.67 points with paired $p=.002244$ and
$6.677\times10^{-5}$. On MovieLens-5/Main/Hard, active AUC changes by at most
0.000163, while all-party local-mean improvements are 0.003849--0.005501 and
connected-party improvements are 0.008456--0.013103; every local and connected
comparison has win/tie/loss 5/0/0 and a 95\% interval above zero.
Same-cardinality subset baselines match or slightly exceed \method{} in mean
active performance at substantially lower communication. These results support
an interface-dependent mechanism claim rather than a universal graph-necessity
claim.
...
is an oracle diagnostic and is excluded from deployable-baseline claims. The
MovieLens five-party headline still emphasizes the communication--accuracy
trade-off rather than a large AUC margin; Tables~\ref{tab:supp-movielens-exchange}
and~\ref{tab:supp-movielens-subset} separately report the mechanism boundary.

The shortlist sweep is exploratory and is interpreted setting by setting.
All $M=\mathrm{all}$ controls exactly reproduce exhaustive selection. The
main-paper scalability claim uses only the nontrivial $N=20,M=20$ and
$N=30,M=40$ settings; the remaining shortlist rows are not used to claim
uniform pruning reliability.
...
fixed except for the explicitly ablated component. The MovieLens closure audit
applies the same intervention to leakage-safe five-party and 15-party caches and
uses ROC-AUC throughout. Direct subset baselines set $q=|\mathcal{S}(E)|$ from
the corresponding default topology state and use validation labels only for
ranking or greedy subset selection; test values are evaluated only after
selection.
...
\paragraph{Accept-evidence closure audit.}
The MovieLens mechanism and top-$M$ prescreening audits are implemented by
\path{experiments/accept_evidence_closure_20260710.py}. The closure run used
Python 3.14, PyTorch 2.12.1+cpu, and PyTorch Geometric 2.8.0. The MovieLens
seed-42 smoke test exactly reproduced ROC-AUC 0.7530094676 from the existing
cache, and all 15 $M=\mathrm{all}$ scalability controls reproduced the
historical exhaustive selector before shortlist results were accepted.
...
\path{experiments/run_nested_weak_scaling.py}; and the MovieLens mechanism and
pruning closure by \path{experiments/accept_evidence_closure_20260710.py}.
The submitted package contains raw CSV/JSON outputs, figure scripts,
supplementary diagnostics, and a usage README.
