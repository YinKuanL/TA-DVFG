...
cheaper and slightly stronger for active-only readout, whereas peer exchange
raises party-local accuracy by 1.85/2.67 points and connected-party accuracy by
4.79/7.16 points. The same boundary reproduces on MovieLens: disabling only peer exchange changes
active AUC by at most 0.00016 but lowers connected-party mean AUC by
0.0085--0.0131. \method{} reaches 0.7516 AUC at 3.20M transmitted scalars. Thus, learned topology is not
claimed to be necessary for one centralized aggregate; its distinct role is
...
communication. A consortium may need both a coordinator-level aggregate and improved local
predictions without exporting hidden states or routing every inference through a
permanent center. The problem is therefore to learn a sparse
...
MovieLens settings, covering communication, deployment, scalability, label
ownership, and frozen-predictor latent-alignment references.
...
aggregated embeddings. These approaches coordinate heterogeneous models through parameters,
optimization, or representations. In contrast, \method{} treats predictors as
persistent local components with no shared architecture, hidden dimension, or
latent semantics; collaboration begins only after each maps its private input
to a compatible task-output space.
...
common encoder family, hidden dimension, or latent coordinate system. We also
...
Does learned sparsity improve over dense, fixed matched-sparse, and
no-collaboration references?
...
comparisons. Oracle Matched in Table~\ref{tab:hgb-main} is a post-hoc test-mean upper
...
uses $k=5$. We audited all 90 headline HGB result rows and verified that their
...
651,970/130,402/162/825,218/297,938. Figure~\ref{fig:movielens-evidence} shows
standalone predictor variation and the AUC--communication trade-off. Best Single reaches 0.7402 AUC with zero communication; paired mechanism
analyses appear in the supplement.

Table~\ref{tab:movielens-main} shows that \method{} reaches 0.7516 ROC-AUC with
3.20M communication, compared with 0.7497/10.00M for Full Mesh and
0.7510/5.20M for Adaptive Pairwise. Because the AUC differences are small, the
five-party result is one of Pareto efficiency rather than a large predictive
gain. In the controlled 15-party Main/Hard stress settings, \method{} reaches
0.7516/0.7498 AUC with 5.2/5.0 links, above Full Mesh and Adaptive Pairwise.

\paragraph{Real-data mechanism boundary.}
Same-state MovieLens-5/Main/Hard ablations hold the selected state fixed and
change only $K=1\rightarrow0$. Active AUC changes by at most 0.00016; all-party
and connected-party mean AUC drop by 0.0038--0.0055 and 0.0085--0.0131,
respectively, with 5/0/0 wins/ties/losses in every setting. On MovieLens-15,
Greedy-Subset-$q$ reaches 0.7524/0.7498 versus 0.7516/0.7498 for \method{} at
62--64\% lower communication. Thus the HGB boundary reproduces: active-only
performance is mainly selection, while peer exchange improves local consumers.
...
We ran a separate five-seed ACM Hard suite with identical freshly trained then
frozen predictors per seed. Historical headline caches were excluded after a
comparability audit found different predictor realizations. Hidden-state export reproduced the original forward path
...
by 4.79/7.16 points. Together with the MovieLens same-state audit, this supports
a cross-benchmark boundary: active-only deployment is mainly participant
...
links, confirming exhaustive scoring as the current bottleneck. Pairwise-utility prescreening cuts evaluations by 88.8\% at $N=20$ with a
$5.2\times$ speedup within 0.10 accuracy points of exhaustive search; at
$N=30$, it cuts 90.5\% with an $8.5\times$ speedup within 0.23 points.
Shortlist size is setting-dependent. Minimum-link and edge-budget sweeps further show that sparse
collaboration can emerge without a forced quota and that $B$ remains an upper
bound; full results appear in the supplement.
...
choices for active-only deployment on both HGB and MovieLens-15. \method{} is
most justified when deployment also requires party-local predictions, because
its sparse links materially improve connected parties across both benchmark
families.
...
selection matches or slightly exceeds \method{} at lower communication on the
audited HGB Hard and MovieLens-15 settings. The learned graph is therefore most
...
information. Third, HGB provides controlled graph-view constructions, while
...
exhaustive candidate scoring remains a scalability bottleneck. Two-stage
prescreening yields $5.2$--$8.5\times$ speedups in the supported 20/30-party
settings, but effective shortlist size is setting-dependent and aggressive
pruning is not uniformly reliable.
