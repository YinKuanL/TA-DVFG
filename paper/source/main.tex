...
choices for active-only deployment, whereas \method{} is most justified when
party-local predictions are also required.
...
information. Compatible output semantics do not ensure calibrated predictors, and our
reliability weighting does not explicitly calibrate party outputs. Third, HGB provides controlled
...
relationship. Because the primary results use only five seeds, non-significant tests should not be interpreted as evidence of equivalence.
...
The review package contains the referenced scripts and raw outputs, while exact
hardware and software-version metadata remain partially preserved.
...
needs only participant selection or a learned peer topology. Direct subset selection is a strong active-only alternative, whereas sparse peer
exchange benefits party-local prediction. Future work should protect messages,
scale topology search, and study the trade-off with latent fusion.
