...
\item We evaluate the method across controlled HGB settings and heterogeneous
MovieLens predictors, including communication, label ownership, scalability,
deployment objectives, and a five-seed frozen-predictor latent-alignment
reference.
...
resulting deployment readout is evaluated. Edge utility is therefore defined by
its effect within the current topology and deployment objective, rather than by
a precomputed pairwise score.
...
\end{align}
All scores are computed only on $V_{\mathrm{topo}}$. Different deployment
objectives can therefore prefer different communication graphs.
...
\caption{HGB Test@BestVal accuracy (\%, mean $\pm$ standard deviation over
...
\caption{Five-party MovieLens predictor interfaces. Parameter counts are
verified by the architecture audit; per-party validation/test AUC appears in
Figure~\ref{fig:movielens-evidence}.}
\label{tab:movielens-heterogeneity}
\begin{tabular}{lllr}
\toprule
Party & Private view & Predictor & Params \\
\midrule
$P_1$ & Rating interactions & MF & \PoneParams \\
$P_2$ & User profiles & MLP & \PtwoParams \\
$P_3$ & Popularity / temporal & MLP & \PthreeParams \\
$P_4$ & Movie metadata & MLP & \PfourParams \\
$P_5$ & Genre similarity & KNN+MLP & \PfiveParams \\
\bottomrule
\end{tabular}
\end{table}

\paragraph{Controlled HGB results.}
...
\caption{ACM Main deployment communication (transmitted scalars). ``Inference
total'' is peer exchange plus readout collection; low-frequency topology-control
traffic is reported separately in the supplementary material.}
\label{tab:communication-main}
\begin{tabular}{lrrr}
\toprule
Method & Peer & Readout & Inference total \\
\midrule
Global Top-$k$ & 0 & 45,375 & 45,375 \\
Oracle Matched & 90,750 & 54,450 & 145,200 \\
...
\begin{figure*}[!t]
\centering
\includegraphics[width=0.4\textwidth]{figures/movielens_pareto_auc_communication.png}
\hfill
\includegraphics[width=0.46\textwidth]{figures/movielens_party_val_test_auc.png}
\caption{MovieLens evidence. Left: ROC-AUC versus total communication for the
five methods reported in Table~\ref{tab:movielens-main} Panel A. Right:
validation and test ROC-AUC of the five heterogeneous local predictors.}
\label{fig:movielens-evidence}
\end{figure*}


\paragraph{MovieLens real-data heterogeneous predictor evaluation.}
Table~\ref{tab:movielens-heterogeneity} makes the five-party interface
heterogeneity explicit. Private views
span interactions, user profiles, temporal statistics, movie metadata, and
genre similarity, while model sizes range from 162 to 825,218 parameters across
MF, MLP, and KNN-augmented predictors. Figure~\ref{fig:movielens-evidence} shows both variation in standalone
predictor quality and the resulting AUC--communication trade-off.
Best Single reaches 0.7402 AUC with zero communication; paired seed-level AUC
and communication deltas are reported in the supplementary material.
...
\begin{figure*}[!t]
\centering
\includegraphics[width=0.4\textwidth]{figures/topology_objective_ablation.pdf}
\hfill
\includegraphics[width=0.45\textwidth]{figures/deployment_tradeoff.png}
\caption{Mechanism and deployment analysis. Left: graph-level scoring versus
pairwise and complementarity-only heuristics. Right: active/local trade-offs for
active, local, and joint topology objectives.}
...
beyond any minimum quota, $B$ remains an upper bound. Full results are reported
in the supplementary material.

\begin{table*}[!t]
\centering
\footnotesize
\caption{Mean MovieLens results over seeds 42--46. Left: five-party AUC and
total communication, including no-collaboration and centralized reference
methods. Right: controlled 15-party weak-predictor stress settings.}
...
Global Top-$k$ is the strongest centralized prediction-selection reference and
is attractive when centralized collection is acceptable and only a single
global readout is required. The mechanism audits sharpen this boundary: direct
same-cardinality subset selection is also a strong lower-cost alternative for
active-only deployment. \method{} is therefore most justified when deployment
also requires party-local predictions, because its sparse links enable peer
exchange that materially improves connected parties.

The latent-alignment study sharpens the same interface distinction. Naive
all-party fusion is strongly affected by weak-party contamination, whereas
validation-based party selection recovers a substantial fraction of the lost
performance. The useful-party-only oracle further shows that latent fusion is
not intrinsically ineffective. The relevant question is therefore not simply
``predictions versus representations,'' but which information interface and
selection mechanism are available at deployment. On ACM Hard, \method{} remains
above the deployable reliability-selected latent reference while requiring only
task-level outputs and producing an explicit sparse peer-to-peer graph.

A selected edge should not be interpreted as a universal semantic similarity or
